    ...
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx in _batch_indices(train_idx, batch_size):
            tensors = _make_batch_tensors(global_x, local_x, scalar_x, y_class, y_binary, batch_idx, torch, device)
            optimizer.zero_grad()
...
def predict_cnn_candidate_views(
    ...
    *,
    ...
    with torch.no_grad():
        outputs = model(
            torch.as_tensor(global_x[None, :, :], dtype=torch.float32, device=device),
            torch.as_tensor(local_x[None, :, :, :], dtype=torch.float32, device=device),
            torch.as_tensor(scalar_x[None, :], dtype=torch.float32, device=device),
        )
        probs_arr = F.softmax(outputs["class_logits"], dim=1).cpu().numpy()[0]
...
def _make_batch_tensors(global_x, local_x, scalar_x, y_class, y_binary, batch_idx, torch, device):
    return {
        "global": torch.as_tensor(global_x[batch_idx], dtype=torch.float32, device=device),
        "local": torch.as_tensor(local_x[batch_idx], dtype=torch.float32, device=device),
